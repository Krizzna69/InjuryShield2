...
def create_visualization(video_path, frame_indices, risk_scores, output_dir, quality='medium'):
    ...
    if file_size_mb > max_size_mb:
        print(f"File size ({file_size_mb:.2f} MB) exceeds target size ({max_size_mb} MB). Attempting further compression...")
        try:
...
def generate_recommendations(risk_factors):
    """Generate training recommendations based on identified risk factors"""
    recommendations = []

    if risk_factors['Knee Valgus']:
    ...
    if risk_factors['Excessive Hip Drop']:
        recommendations.append(
    ...
    if risk_factors['Improper Stride Width']:
    ...
    if risk_factors['Suboptimal Foot Strike']:
    ...
    if not recommendations:
        recommendations.append("Continue current training regimen with focus on gradual progression")
        recommendations.append("Maintain good strength training habits for injury prevention")
